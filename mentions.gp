...
stats FILENAME nooutput # calc stats to determine number of indices
TITLE = system("awk 'NR==2 {print substr($0,4); exit} ' < ".FILENAME)
set title TITLE
...
set yrange [0:*]
set xtics format "%b'%y" # seems more flexible than set xtics x "..."
set xtics nomirror
...
plot for [INDEX = 0:STATS_blocks-1] FILENAME index INDEX \
  skip 2 using 1:2 with line lw 3 title columnhead(1)
